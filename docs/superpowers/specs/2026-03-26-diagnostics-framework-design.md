...
本次重构完成后，必须把 diagnostics 支持的所有有效命令组合在测试数据库上完整跑一遍。这既是 diagnostics 自身的验收，也是对现有业务代码的一次全面回归检查。

约束：

1. 验收必须使用测试数据库，不能在生产数据库上完成此项全量跑测。
2. 需要覆盖所有有效模式与参数组合，而不只是 `collection` / `reports` / `full` 的 happy path。
3. 对于会写入的组合，必须按 spec 中的写入门禁显式执行。
4. 对于应被拒绝的非法参数组合，也必须验证其拒绝行为正确。

验收目标：

1. diagnostics 脚本本身没有参数解析、门禁、结果聚合、输出格式问题。
2. 共享 orchestrator 与报表 runner 在真实运行中行为正确。
3. 通过 diagnostics 触发的一系列业务流程没有隐藏故障。

如果在这轮全量验收中发现业务代码问题：

1. 不能只把问题记录下来后结束工作。
2. 需要先使用 subagent 对问题做规划和思考，明确修复边界、风险和改动方案。
3. 在规划清楚后再实施业务代码修复。
4. 修复后重新运行受影响的 diagnostics 命令组合，直到结果稳定。

### 最终汇报要求

重构与全量验收完成后，必须输出详细汇报，至少包含：

1. 本次实现了哪些 diagnostics 能力
2. 实际跑过哪些命令组合
3. 哪些组合通过、哪些组合失败、失败原因是什么
4. 是否在验收过程中发现并修复了业务代码问题
5. 修复了哪些问题、影响范围是什么、如何验证
6. 仍然存在的风险、限制和后续建议
