...
export function createRun(db: Database, run: Required<Pick<RunRecord, "id" | "mode" | "status">> & {
  ...
  ).run(run.id, run.mode, run.sourceSelectionJson, run.paramsJson, run.status, run.createdAt);
}
